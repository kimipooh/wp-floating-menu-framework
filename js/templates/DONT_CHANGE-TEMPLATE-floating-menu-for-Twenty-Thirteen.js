// Sample for Twenty Thirteen (System is same as Twenty Twelve)
// @Kimiya Kitani (kimipooh) 17/09/2014

// User Settings
var name = "#navbar"; // Div/Nav ID which you want to set up Floating Menu. 
...
jQuery(document).ready(function(){
	...
	jQuery(window).scroll(function () { 
		offset2 = jQuery(window).scrollTop();
		if(name_screenTop < offset2){
			var offset_num = parseInt(jQuery(window).scrollTop());
			offset = offset_num +"px";
			jQuery(name).animate({top:offset},{duration:500,queue:false});
			if(flag == 0) flag = 1;
		}else{
			if(flag == 1){
				var offset_num = name_screenTop-adminbar_height;
				offset = offset_num +"px";
				jQuery(name).animate({top:offset},{duration:500,queue:false});
				flag = 0;
			}
